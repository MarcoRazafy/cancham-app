...
/**
 * Adresse d'un état de l'agenda. Tout vit dans l'URL : un lien partagé ou un
 * retour arrière retrouve la même vue, sans état côté client.
 */
function lien({ vue, date, jour, voir }: Etat): string {
  const q = new URLSearchParams();
  if (vue !== "mois") q.set("vue", vue);
  q.set("date", date);
  if (jour) q.set("jour", jour);
  if (voir.length < TYPES_ELEMENT.length) {
    q.set("voir", voir.length ? voir.join(",") : "aucun");
  }
  return `/membre/agenda?${q}`;
}

function lireVoir(v: string | undefined): TypeElement[] {
  if (!v) return TYPES_ELEMENT.map((t) => t.cle);
  const demandes = v.split(",");
  return TYPES_ELEMENT.map((t) => t.cle).filter((t) => demandes.includes(t));
}
...
export default async function AgendaPage({
  ...
  const tous = await getAgenda(
    { userId: user.id, memberId: user.memberId },
    du,
    au,
  );
  ...
  return (
    <>
      ...
      >
        Les événements de la chambre, vos échéances de cotisation et de
        factures, et vos rappels, au même endroit.
      </ViewHead>
